...
const Employees = () => {
  ...
  const handleCreateEmployee = async (employeeData) => {
    try {
      ...
      if (error.response?.status === 403) {
        toast.error('You do not have permission to create employees. Manager or Admin role required.');
      } else if (error.response?.status === 401) {
        toast.error('Please log in again');
        // Перенаправление на логин уже обработано в API interceptor
      } else {
  ...
  const handleUpdateEmployee = async (id, employeeData) => {
    try {
      ...
      if (error.response?.status === 403) {
        toast.error('You do not have permission to update employees. Manager or Admin role required.');
      } else if (error.response?.status === 401) {
        toast.error('Please log in again');
        // Перенаправление на логин уже обработано в API interceptor
      } else {
  ...
  const handleDeleteEmployee = async (id) => {
    ...
    if (window.confirm(`Are you sure you want to deactivate ${employeeName}?\n\nThis action will:\n- Remove them from all projects\n- Archive their profile\n- Keep their data for reporting\n\nThis action cannot be undone.`)) {
      try {
        ...
        if (error.response?.status === 403) {
          toast.error('You do not have permission to delete employees. Admin role required.');
        } else if (error.response?.status === 401) {
          toast.error('Please log in again');
          // Перенаправление на логин уже обработано в API interceptor
        } else {
